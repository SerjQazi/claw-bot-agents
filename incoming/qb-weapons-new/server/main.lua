...
--         -- sync UI
--         TriggerClientEvent('qb-inventory:client:UpdatePlayerInventory', src, Player.PlayerData.items)
--     end
-- end)

RegisterNetEvent('qb-weapons:server:UpdateWeaponAmmo', function(weaponName, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    for slot, item in pairs(Player.PlayerData.items) do
        if item and item.name == weaponName then
            ...
            -- print("🔥 SAVED:", weaponName, "AMMO:", amount)
...
local function EquipWeaponTint(source, tintIndex, item, isMK2)
    ...
    -- Custom inventory sync fix
        TriggerClientEvent('qb-inventory:client:UpdatePlayerInventory', source, Player.PlayerData.items)
